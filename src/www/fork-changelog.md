...
- Fixed AltGr (e.g. `AltGr+Q` for `@` on German layouts) producing no character at all when typed live. AltGr fires `KeyboardEvent.key === 'AltGraph'`, which the modifier check never matched, so it was silently dropped as an unsupported key. On Windows, AltGr also triggers a phantom Left-Ctrl keydown just before the real event (a known browser/OS quirk); since AltGr itself was dropped, the remote received a bare Ctrl press instead of AltGr - explaining why not even a plain character was typed. AltGr is now recognized and forwarded as the HID Right-Alt modifier, with the phantom Ctrl bit cleared alongside it.
- Fixed Paste to Remote silently dropping umlauts (`ü`, `ö`, `ä`, `ß`) and sending the wrong symbol for several punctuation characters (`< > [ ] { } | \ ~` and others) when the remote uses a German (QWERTZ) layout. Added a "Paste Keyboard Layout" setting with US and German character tables, each mapping every character to its correct physical key and modifiers (including AltGr) for that specific remote layout — since pasted text has no live `KeyboardEvent` to read a physical position from, unlike live typing above. On the German table, the two dead-key characters (´ acute, ^ circumflex) and the backtick `` ` `` (which shares the dead-grave key) still can't be sent as a single keypress and are reported as skipped, same as any other unsupported character.
- Moved the "Paste Keyboard Layout" picker from Settings > Keyboard & Mouse into the Paste to Remote popup itself (as a compact dropdown next to the title), since it's easy to miss buried in Settings when it's specific to this one feature. Still persisted the same way as before (shared with the rest of the app's settings in `localStorage`), just relocated.
...
- Fixed Paste to Remote being extremely slow (roughly one second per character) compared to live typing. Sending previously re-pressed and re-released Shift/AltGr around every single character and added a fixed 30ms pause after each one; it now only toggles a modifier when the required state actually changes between characters (matching how live typing already behaves - one physical Shift press covers many letters, not one per letter) and no longer adds an artificial per-character delay, while still guaranteeing no modifier is left held on the remote if sending is cancelled or fails partway through.
